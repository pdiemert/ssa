 ...
 *
 * headers                  An object specifying name/values for headers to pass
 *
 * follow                   A Javascript expression string to evaluate which returns either a URL or an object used for making the next request
 *                          Within scope of the expression is an object called 'data' which is the result of the request
 *                          specified by the dependsOn property.  If dependsOn is an array, the first dependency will be used.
 *                          The get http method will be used.
...
function SuiteRunner(suite, options)
{
    ...
    function checkTest(t, res, args)
    {
        ...
        for (var i = 0; i < isa.length; i++)
        {
            ...
            var res = checkTest(tmp, res, args);

            if (res != null)
                return res;
        }

        var code = prop(t, 'expectCode', 200);
        var jsono = prop(t, 'expectJSON');
        var type = prop(t, 'expectType');
    ...
    function handleHttp(t, err, res, body)
    {
        ...
        finishTest(t, checkTest(t, res, [obj]));
    }
    ...
    function startTest(t)
    {
        ...
        try
        {
            ...
            var startSync = prop(t, 'setupSync');
            var data = propOrFunc(t, 'data');
            var follow = prop(t, 'follow');
            var heads = prop(t, 'headers');

            if (!td.started)
            {
                var count = propOrFunc(t, 'repeat', 0);
                td.repeat = count;
                td.started = true;
            }
            ...
            if (follow && t.dependsOn)
            {
                ...
                {
                    url = f[prop(_options, 'resourceProp', ':self')];
                    accept = f[prop(_options, 'typeProp', ':type')];

                    t.isJSON = typeIsJSON(accept);
                }
            }
