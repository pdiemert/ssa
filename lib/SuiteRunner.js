...
*/
    ...
    {
        return genCallback(f, t, (new Date()).getTime());
    }
    ...
    function handleHttp(t, ts, err, res, body)
    {
    ...
    function startTest(t)
    {
        ...
        try
        {
            ...
            if (follow && t.dependsOn)
            {
                ...
                if (follow.substr(0, 6) == 'jpath:')
                {
                    var path = follow.substr(6);
                ...
                {
                    url = f[testProp(_options, 'resourceProp', ':self')];
                    accept = f[testProp(_options, 'typeProp', ':type')];
