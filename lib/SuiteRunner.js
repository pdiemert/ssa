...
require('./array.js');
require('./string.js');
var p_req = require('request');
var p_assert = require('node-assert-extras');
...
var p_https = require('https');
p_https.globalAgent.options.secureProtocol = 'SSLv3_method';
p_https.globalAgent.options.rejectUnauthorized = false;
//p_https.globalAgent.options.requestCert = true;
 ...
 * timeout					Default timeout for callbacks, default is 5000
 * reqtime					Default timeout for requests, default is 5000
...
function SuiteRunner(suite, options)
{
    ...
    var _requests = [];
    var _quitting = false;
	var _ignoreResponse = _options.ignoreResponse || false;
	...
	// Callback should receive err, res, body
	function callHttp(t, params, cb)
	{
		if (USE_AJAX)
		{
			var url = new p_url.URL(params.uri);
			var fSSL = url.scheme == 'https';
			return ajax(fSSL, params.method, params.json, url.host, fSSL ? 443 : url.port, url.path, params.headers, cb);
		}
		else
		{
			var reqid = s_reqid++;

			s_waiting[reqid] = true;
			setTimeout(function()
			{
				if (s_waiting[reqid])
				{
					// Timed out! Error
					delete s_waiting[reqid];

					return cb && cb(new Error('timeout'));
				}
			}, 5000);

			return p_req(params, function(err,res,body)
			{
				if (s_waiting[reqid])
				{
					delete s_waiting[reqid];
    ...
    function handleHttp(t, ts, params, retries, err, res, body)
    {
        ...
        if (err)
        {
            ...
            // Retry if avail
            if (_options.retry && (retries > 0 || ihang != -1))
            {
        ...
        var obj;
        if ((responseIsJSON(res) || t.isJSON) && p_u.isString(body))
        {
            obj = JSON.parse(body);
            if (t.dump)
                p_eyes.inspect(obj);
        ...
        if (_options.reqclock)
        {
            ...
            if (c)
            {
                if (!(c in _options.reqclock))
                    _options.reqclock[c] = {count:0, elapsed:0,hist:[]};
    ...
    function callfuncA(f, t, a)
    {
        ...
        self.__defineGetter__('callback', function ()
        {
			...
			var fnc = genCallback(handleCallbacks, thistest);
			
			var timeout = setTimeout(function()
			{
				if (fnc)
				{
					// taking too long error
					thistest.callbackCount--;
					finishTest(thistest, 'timeout');
				}
			}, _options.timeout || 5000);
			
			return function()
			{
				clockCall(thistest, start);
				...
				if (fnc)
				{
					fnc.apply(null, [].splice.call(arguments,0));
					fnc = null;
				}
			};
        });
	...
	// Calls cb with err, res, body
	// Returns http req
	function ajax(fSSL, meth, msg, host, port, path, headers, cb)
	{

		var buff;
		var ct;
		var http = fSSL ? p_https : p_http;

		if (msg)
		{
			buff = JSON.stringify(msg);
			ct = 'application/json';
		}
		...
		if (headers)
		{
			for(var hdr in headers)
				options.headers[hdr] = headers[hdr];
		}
...
//			options.agent = false;
		...
		setTimeout(function()
		{
			if (s_waiting[reqid])
			{
				console.log('No callback for %d', reqid);
				delete s_waiting[reqid];
				cb(new Error('Timeout waiting for reply'));
			}
		}, 30000);

		// expect output to be application/x-msgpack
		var buffOut;
    ...
    function startTest(t)
    {
        ...
        try
        {
            ...
            if (url)
            {
                ...
                if (data)
                {
                    if ((p_u.isObject(data) || Array.isArray(data)) && !Buffer.isBuffer(data))
                        params.json = data;
                    else
                        params.body = data;
                }
				...
				_requests.push(callHttp(t, params,
					 function(err, res, body)
					 {
						 handleHttp(t, Date.now(), params, 0, err, res, body);
					 }));

					//genHttpFunc(t, handleHttp, params, 0)));
            }
    ...
    this.run = function (fnc)
    {
        ...
        try
        {
            ...
            if (!tests.length)
            {
	            // Tiny hack:  all tests where invalid so we need to abort but in order to have a failure
	            //             count we need to have at least one test.  Add one for this.
	            _testcount = 1;
	            _completed = 1;
                throw 'Either no tests were found or no tests without dependencies exist, unable to run suite';
            }
